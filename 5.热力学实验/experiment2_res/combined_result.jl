...
# 使用for循环遍历1到10的文件名
for i in 1:10
    file_path = joinpath(file_directory, "result_$i.csv")
    try
...
# 将合并后的数据写入一个新的CSV文件
CSV.write("combined_result.csv", combined_data)
...
open("combined_result.md", "w") do io
...
println("处理完成，结果已写入 combined_result.md 文件。")
